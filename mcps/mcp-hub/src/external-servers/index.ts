...
import type { ExternalToolMeta, ServerConfig } from "../types.js";
...
export class ExternalServersManager {
	...
	private validateConfig(config: ServerConfig): void {
		if (
			...
			throw new Error(
				`Server '${config.id}' with ${config.transport} transport requires a url`,
			);
		}
		if (config.transport === "stdio" && !config.command) {
			throw new Error(
				`Server '${config.id}' with stdio transport requires a command`,
			);
		}
	}
	...
	async closeAll(): Promise<void> {
		for (const [serverId, client] of this.clients) {
			try {
				...
				console.error(
					`Failed to close client for server '${serverId}':`,
					error,
				);
			}
		}
