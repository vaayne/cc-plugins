...
// TestHandleSearchTool_Success verifies successful search
func TestHandleSearchTool_Success(t *testing.T) {
	logger := zaptest.NewLogger(t)
	registry := NewBuiltinToolRegistry(logger)
	...
	registry.RegisterTool(config.BuiltinTool{
		Name:        "search",
	...
	assert.Equal(t, "search", response.Tools[0].Name)
	assert.Equal(t, "builtin", response.Tools[0].Server)
}

// TestHandleSearchTool_CaseInsensitive verifies case-insensitive search
func TestHandleSearchTool_CaseInsensitive(t *testing.T) {
	logger := zaptest.NewLogger(t)
	registry := NewBuiltinToolRegistry(logger)
	manager := client.NewManager(logger)
	defer manager.DisconnectAll()
	...
	assert.Len(t, response.Tools, 1)
	assert.Equal(t, "MyTool", response.Tools[0].Name)
}

// TestHandleSearchTool_DescriptionMatch verifies search matches descriptions
func TestHandleSearchTool_DescriptionMatch(t *testing.T) {
	logger := zaptest.NewLogger(t)
	registry := NewBuiltinToolRegistry(logger)
	manager := client.NewManager(logger)
	defer manager.DisconnectAll()
	...
	registry.RegisterTool(config.BuiltinTool{
		Name:        "tool1",
		Description: "This tool handles file operations",
	})
	...
	assert.Len(t, response.Tools, 1)
	assert.Equal(t, "tool1", response.Tools[0].Name)
}
