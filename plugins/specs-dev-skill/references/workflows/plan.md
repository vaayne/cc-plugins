# Feature Planning with Reviewer Subagent

## When to use this reference

Use this reference when the skill is in the planning phase (requirements discussion, plan creation, or plan review) and a formal plan + tasks session should be produced.
...
| 3. Reviewer Review         | Send the full plan to the `reviewer` subagent, integrate its feedback, repeat if needed (≤3 rounds).             | Reviewer confirms the plan is solid and feedback is incorporated.     |
...
- **Gate B (end of Phase 3):** Confirm reviewer feedback addressed and user satisfied before writing files.
...
- [ ] Reviewer feedback captured, decisions documented.
...
### Phase 3 – Reviewer Review & Refinement

1. Invoke the `reviewer` subagent with the full plan.
2. Request feedback on completeness, technical soundness, security/performance implications, and risk areas.
3. Integrate reviewer feedback; clarify or adjust sections as needed.
4. Iterate (maximum of three passes) until the reviewer indicates the plan is comprehensive and ready.

**Best Practices:**

- Always send the entire plan, not a summary.
- Be explicit about the angle of review (security, performance, edge cases, etc.).
- Note reviewer recommendations in the plan so decisions remain traceable.
...
3. Confirm the session path with the user, summarize next steps, and state that the implementation phase reads this directory.
...
- Encourage the user to treat reviewer feedback as blocking until addressed.
...
- **Reviewer feedback feels generic:** Provide sharper prompts outlining stack, modules, and risk areas.
...
- **Session directory missing:** Ensure Phase 4 runs and paths are correct; recreate if necessary before moving to implementation.

### Tips for Excellent Plans

1. Patience in Phase 1 pays off—better questions reduce redo loops later.
2. Don’t rush the plan; specificity makes implementation straightforward.
3. Trust reviewer feedback and document the adjustments you make.
