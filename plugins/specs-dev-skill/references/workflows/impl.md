# Feature Implementation with Reviewer Subagent

## When to use this reference

Use this reference when the skill has an approved plan and is in the implementation phase, with work tracked in `.agents/sessions/{YYYY-MM-DD-feature}/`.

You are executing a disciplined implementation workflow that keeps the reviewer subagent in the loop and lands focused, validated commits.
...
| 3. Iterative Implementation | For each task: mark in progress, implement via worker subagent, run tests, collect reviewer feedback, commit, update docs. | All tasks complete, reviewer feedback addressed, commits passing.                               |
...
2. Launch implementation subagent (`worker`) with explicit instructions.
3. Run required tests or checks.
4. Request reviewer subagent feedback (`reviewer`) on the diff or files.
...
1. Verify the session path exists and contains `plan.md` and `tasks.md`.
...
2. **Implement** – Invoke the `worker` subagent with a detailed prompt: objective, files, acceptance tests, patterns to follow.
3. **Validate** – Run unit/integration tests relevant to the change. Document commands you run.
4. **Reviewer Review** – Invoke `reviewer` with the diff or affected files. Request severity-ranked findings covering bugs, security/performance issues, and regressions.
...
- [ ] No pending reviewer feedback or unanswered comments.
...
- **Implementation agent:** `worker` subagent (all implementation tasks), or `debugger` for troubleshooting failing tests.
- **Review agent:** `reviewer` subagent; include stack details, modules touched, and request severity-ranked output.
...
- **No session folder:** Confirm the path argument and create the folder using the planning workflow before retrying.
- **Task too large:** Split it into smaller vertical slices; update TodoWrite and `tasks.md` accordingly.
- **Reviewer requests major rework:** Pause, revisit the plan with the user, and update `plan.md` before continuing.
- **Persistent test failures:** Switch to the `debugger` agent or request deeper reviewer analysis targeting the failing module.
...
- Encourage treating reviewer feedback as blocking until resolved.

Delivering consistent updates and respecting these quality gates keeps implementations predictable and merge-ready.
